...
export default class IntersectionObserver {
		...
		if (options !== undefined && options !== null) {
			...
			if (options.rootMargin !== undefined && options.rootMargin !== null) {
				if (typeof options.rootMargin !== 'string') {
					throw new window.TypeError(
						`Failed to construct 'IntersectionObserver': Failed to read the 'rootMargin' property from 'IntersectionObserverInit': The provided value is not of type 'DOMString'.`
					);
				}
				const parsed = this.#parseRootMargin(options.rootMargin);
	...
	public unobserve(target: Element): void {
		...
		this.#pendingEntries = this.#pendingEntries.filter((entry) => entry.target !== target);
