...
describe('IntersectionObserver', () => {
	...
	describe('unobserve()', () => {
		it('Stops future entries for that target.', async () => {
			...
			// Cross thresholds so remaining observed targets emit new entries.
			rectA = { x: 0, y: 718, width: 100, height: 100 };
			rectB = { x: 0, y: 718, width: 100, height: 100 };

			await waitForFrame();
			await waitForDelivery();

			const targetsAfterUnobserve = batches
				.slice(1)
				.flatMap((batch) => batch.map((entry) => entry.target));
	...
	describe('intersection calculations', () => {
		...
		it('Triggers new entries when a target crosses a threshold.', async () => {
			...
			// Place box half outside to get 0.5 ratio: visible height = 768-718 = 50
